...
if (isset($_POST['dltpr'])) {
	...
	$id=$_REQUEST['pid'];
	$getmax = "DELETE FROM product WHERE pid=$id";
	$conn -> query($getmax);
	
	$getmax = "SELECT pid,title,price,btime FROM product ORDER BY pid";
...
elseif (isset($_POST['btime' ])) {
	...
	$getmax = "UPDATE product SET btime='$btime' WHERE pid=$id";
	$result = $conn -> query($getmax);
